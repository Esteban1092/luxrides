...
// Notificaciones push: solo mostrar la notificación visual, el sistema usará el sonido predeterminado
// Notificaciones push: mostrar notificación visual con vibración, sonido del sistema y opciones avanzadas
messaging.onBackgroundMessage((payload) => {
  const notification = payload?.notification || {};
  const data = payload?.data || {};
  const notificationTitle = notification.title || data.title || 'Nuevo viaje LuxRides';
  const notificationOptions = {
    body: notification.body || data.body || 'Hay un servicio esperando.',
    icon: data.icon || './luxrides-driver-icon.svg',
    badge: data.badge || './luxrides-driver-icon.svg',
